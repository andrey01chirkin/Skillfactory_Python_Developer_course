...
В качестве значения по умолчанию у этих аргументов должно быть None. Также __init__() должен создавать локальные свойства в экземпляре с соответствующими именами.

Реализуйте методы:
- set_attributes(), который принимает словарь с произвольным числом записей, которые представлены в нём в виде имя_атрибута: значение и которые вам необходимо обновить в классе (в словаре может быть произвольное значение записей, например только для name и age).
